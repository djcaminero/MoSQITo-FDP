...
def loudness_graphics(signal, validation=False):
    """ Same function as comp_loudness, but for testing and validation purposes.
    ...
    om_time = np.arange(len(signal_filtered)) / fs

    plt.figure(figsize=(10, 5))
    plt.plot(om_time, signal_filtered, label='Tono puro, 40 dB SPL, 1000 Hz')
    ...
    plt.title('Señal filtrada por el oído externo y medio/interno')
    ...
    for band_number in range(53):
        ...
        if band_number == 17:
            ...
            bp_time = np.arange(len(band_pass_signal)) / fs

            plt.figure(figsize=(10, 5))
            plt.plot(bp_time, band_pass_signal, label='Señal filtrada, 40 dB SPL, 1000 Hz')
            plt.legend(bbox_to_anchor=(1.04, 0.5), borderaxespad=0, loc='center left', ncol=1)
            plt.subplots_adjust(right=0.65)
            ...
            plt.xlabel('Tiempo [s]')
            plt.ylabel('Amplitud [Pa]')
            plt.title('Señal filtrada por el BFA, banda de 1 kHz')
            plt.grid(which='both', linestyle='-', color='grey')
            plt.show()
        ...
        if band_number == 17:
            ...
            bp_time_rect = np.arange(len(band_pass_signal_hr)) / fs

            plt.figure(figsize=(10, 5))
            plt.plot(bp_time_rect, band_pass_signal_hr, label='Señal rectificada, 40 dB SPL, 1000 Hz')
            plt.legend(bbox_to_anchor=(1.04, 0.5), borderaxespad=0, loc='center left', ncol=1)
            plt.subplots_adjust(right=0.65)
        ...
        for l_number, l_block in enumerate(block_array):
            ...
            if l_number > 0:
                ...
                if band_number == 17 and l_number == 40:
                    ...
                    plt.figure(figsize=(10, 5))
                    plt.plot(bp_time_rect, band_pass_signal_hr, label='Señal rectificada, 40 dB SPL, 1000 Hz')
                    plt.plot(block_time, l_block, label='Señal segmentada, 40 dB SPL, 1000 Hz')
                    plt.legend(bbox_to_anchor=(1.04, 0.5), borderaxespad=0, loc='center left', ncol=1)
                    plt.subplots_adjust(right=0.65)
                    ...
                    plt.xlabel('Tiempo [s]')
                    plt.ylabel('Amplitud [Pa]')
                    plt.title('Señal segmentada, banda de 1 kHz')
                    plt.grid(which='both', linestyle='-', color='grey')
                    plt.show()
        ...
        print('Banda: ' + str(band_number) + ', Número de bloques: ' + str(len(band_blocks_array)))
